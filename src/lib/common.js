...
export async function processUrl(url) {
  ...
  if (party == "sun") {
    ...
    config.urlForm =
      "https://docs.google.com/forms/d/e/1FAIpQLScg0ISOTwggQgsRcJqu1cLv7Jyfo8HtARbtL-BvCSi4niv1yg/viewform";
  ...
  if (!alias) {
    json.values = uniqByKeepLast(json.values, (item) =>
      normalize(item[colName])
    );
  }
  ...
  for (const [i, item] of json?.values?.entries()) {
    ...
    if (
      ...
      const addSponsor =
        headers[colMessage].includes("협찬") && !rawMessage.includes("협찬");
    ...
    const randomIndex = String(Math.floor(random() * 32)).padStart(2, "0");
    let backgroundImage = `/img/grinemo/${randomIndex}.png`;
