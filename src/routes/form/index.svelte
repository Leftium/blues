<script>
  ...
  const SUNDAY_BLUES_URLS = [
    ...
    "https://docs.google.com/forms/d/e/1FAIpQLSdEpJubLzbcgAk2Ism5qwhDpSNA1D_P58HwsDitx0GCD5b3Qg/formResponse",
  ];
...
<main>
  ...
  <div class="info">
    ...
    {#if formParams.length}
      <center>
        <a href="#" on:click|preventDefault={scrollToForm}
          >신청 양식 바로 가기</a
        >
      </center>
    {/if}
  ...
  {#if formParams.length}
    <form id="form" bind:this={form} on:submit|preventDefault={handleSubmit}>
      ...
      {#each formParams as formParam}
        {#if !(formParam.name == "포토그래퍼 맥반석님")}
          <!-- Skip extraneous input. -->
        ...
        <Row>
          <Col class="col-8">
            <Alert
              ...
              color={alertColor}
              isOpen={!!submitResultMessage}>{@html submitResultMessage}</Alert
            >
          </Col>
          <Col class="col-4">
            <Button
